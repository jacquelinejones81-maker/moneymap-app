...
export default function PinLogin({ onSuccess, onNewUser }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        setPin(['', '', '', '']); setError(''); clearInterval(interval);
  ...
  const handleEmailNext = () => {
    ...
    if (!accounts[email.toLowerCase()]) { setError("We don't recognize that email. Are you a new user?"); return; }
  ...
  const verifyPin = (enteredPin) => {
    ...
    const expected = btoa(enteredPin + email.toLowerCase());
    if (account?.pin === expected) {
      const leads = JSON.parse(localStorage.getItem('mm_leads') || '[]');
      const lead = leads.find(l => l.email?.toLowerCase() === email.toLowerCase()) || { email, name: account.name };
      setAttempts(0); onSuccess(lead);
    } else {
      const newAttempts = attempts + 1; setAttempts(newAttempts);
      setPin(['', '', '', '']); setShake(true);
      setTimeout(() => { setShake(false); if (newAttempts < MAX_ATTEMPTS) inputRefs.current[0]?.focus(); }, 600);
      if (newAttempts >= MAX_ATTEMPTS) {
        const until = Date.now() + LOCKOUT_MINUTES * 60 * 1000;
        setLockedUntil(until); setError(`Too many incorrect attempts. Locked for ${LOCKOUT_MINUTES} minutes.`);
      } else {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
      <div className="slide-up" style={{ background: 'var(--navy-card)', border: '1px solid var(--navy-border)', borderRadius: 'var(--radius-xl)', padding: '2.5rem 2rem', maxWidth: 400, width: '100%', textAlign: 'center' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, marginBottom: 8 }}>
          {isLocked ? 'Account Locked' : step === 'email' ? 'Welcome back' : 'Enter your PIN'}
        </h2>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: '1.75rem' }}>
          {isLocked ? `Too many incorrect attempts. Try again in ${fmtCountdown()}.` : step === 'email' ? 'Enter your email to access your budget dashboard.' : `Enter the 4-digit PIN for ${email}`}
        </p>
        {!isLocked && step === 'email' && (
          <>
            <input type="email" placeholder="your@email.com" value={email} onChange={e => { setEmail(e.target.value); setError(''); }} onKeyDown={e => e.key === 'Enter' && handleEmailNext()} style={{ marginBottom: 12, textAlign: 'center' }} autoFocus />
            {error && <div className="alert-box alert-danger" style={{ marginBottom: 12, textAlign: 'left' }}>{error}</div>}
            <button className="btn-gold" style={{ width: '100%', marginBottom: 12 }} onClick={handleEmailNext}>Continue →</button>
          </>
        )}
        {!isLocked && step === 'pin' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginBottom: '1.25rem' }}>
              {pin.map((d, i) => (<div key={i} style={{ width: 14, height: 14, borderRadius: '50%', background: d ? 'var(--gold)' : 'transparent', border: `2px solid ${d ? 'var(--gold)' : 'var(--navy-border)'}`, transition: 'all 0.15s ease' }} />))}
            </div>
            <div className={shake ? 'shake' : ''} style={{ display: 'flex', justifyContent: 'center', gap: 12, marginBottom: '1.25rem' }}>
              {pin.map((d, i) => (
                <input key={i} ref={el => inputRefs.current[i] = el} type="password" inputMode="numeric" maxLength={1} value={d}
                  ...
                  onChange={e => handleDigit(e.target.value, i)} onKeyDown={e => handleKeyDown(e, i)} disabled={isLocked} />
              ))}
            </div>
            {error && <div className="alert-box alert-danger" style={{ marginBottom: 14, textAlign: 'left' }}>{error}</div>}
            <button className="btn-outline" style={{ fontSize: 12, width: '100%' }} onClick={() => { setStep('email'); setPin(['','','','']); setError(''); setAttempts(0); }}>← Use a different email</button>
          </>
        )}
        {isLocked && <div style={{ fontSize: 48, fontFamily: 'var(--font-display)', fontWeight: 800, color: '#f87171', margin: '1rem 0' }}>{fmtCountdown()}</div>}
        <div style={{ borderTop: '1px solid var(--navy-border)', marginTop: '1.5rem', paddingTop: '1.25rem' }}>
          <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>New here? <button onClick={onNewUser} style={{ background: 'none', border: 'none', color: 'var(--gold)', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>Get free access →</button></p>
          <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6 }}><a href="#admin" style={{ color: 'var(--text-muted)', textDecoration: 'none' }} onClick={() => window.location.reload()}>Admin login</a></p>
        </div>
      </div>
