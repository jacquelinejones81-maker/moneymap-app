...
export default function LandingPage({ onSubmit }) {
  ...
  const validate = () => {
    ...
    if (!form.phone.trim() || form.phone.replace(/\D/g, '').length < 10) e.phone = 'A valid phone number is required';
    return e;
  };
  ...
  // Once checked, the box locks — no unchecking allowed
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.1fr) minmax(0,0.9fr)', gap: '2rem', alignItems: 'start' }}>
          ...
          <div className="fade-in">
            <div style={{ marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: 18, fontFamily: 'var(--font-display)', marginBottom: '1rem', color: 'var(--text-primary)' }}>
          ...
          <div className="slide-up" style={{ background: 'var(--navy-card)', border: '1px solid var(--navy-border)', borderRadius: 'var(--radius-xl)', padding: '2rem' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 20, marginBottom: '0.375rem', textAlign: 'center' }}>
              Get Free Access
            </h3>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center', marginBottom: '1.5rem' }}>
            ...
            {/* Financial Review — locks once checked */}
            <div style={{
              background: wantsReview ? 'rgba(14,165,160,0.1)' : 'rgba(14,165,160,0.06)',
              border: `1px solid ${wantsReview ? 'rgba(14,165,160,0.5)' : 'rgba(14,165,160,0.2)'}`,
              borderRadius: 'var(--radius-md)',
              padding: '14px',
              marginBottom: 16,
              transition: 'all 0.3s ease',
            }}>
              <div style={{ fontSize: 12, color: 'var(--teal-light)', fontWeight: 600, marginBottom: 5, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ textAlign: 'center', marginTop: '3rem' }}>
          <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            Already have access?{' '}
            <a href="#admin" style={{ color: 'var(--gold)', textDecoration: 'none' }} onClick={() => window.location.reload()}>
              Admin login
            </a>
          </p>
        </div>
        ...
        <div className="modal-overlay">
          <div className="modal-box slide-up" style={{ maxWidth: 540 }}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 26, marginBottom: 10, lineHeight: 1.2 }}>
                You're In, {submittedLead?.name?.split(' ')[0]}!
              </h2>
              <p style={{ color: 'var(--text-secondary)', fontSize: 14, lineHeight: 1.7 }}>
                Your financial transformation starts <em>right now</em>. You're about to have a tool that shows you exactly where every dollar goes — and a clear path to becoming debt-free.
              </p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: '1.5rem' }}>
              ...
              <div style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 'var(--radius-md)', padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                    Your free <strong style={{ color: 'var(--text-primary)' }}>"How Money Works"</strong> book is on its way, plus exclusive money tips sent straight to your inbox.
                  </div>
                </div>
              </div>

              {/* Financial Review block — only if opted in */}
              {wantsReview && (
                <div style={{ background: 'rgba(14,165,160,0.08)', border: '1px solid rgba(14,165,160,0.35)', borderRadius: 'var(--radius-md)', padding: '16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: 22, flexShrink: 0 }}>📅</span>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--teal-light)', marginBottom: 5 }}>
                        Your complimentary financial review is confirmed ✓
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                        Most people retire on someone else's timeline because they never knew their number. <strong style={{ color: 'var(--text-primary)' }}>We're going to change that for you.</strong> In one free session, we'll map out exactly what you need saved, by when, and build a real plan to get you there.
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Self-schedule CTA */}
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--teal-light)', marginBottom: 8, textAlign: 'center' }}>
                      Want to lock in your spot right now?
                    </div>
                    <a
                      href={CALENDLY_URL}
                      target="_blank"
                  ...
                  {/* Fallback — if they don't self-schedule */}
              ...
              <div style={{ background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.15)', borderRadius: 'var(--radius-md)', padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                <span style={{ fontSize: 20, flexShrink: 0 }}>🚀</span>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                  <strong style={{ color: '#4ade80' }}>Your dashboard is ready.</strong> Set your beginning balance, add your debts, and log your first transactions today. The sooner you start, the sooner you win.
                </div>
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--radius-md)', padding: '12px 16px', marginBottom: '1.25rem' }}>
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  I agree to receive the free <strong style={{ color: 'var(--text-primary)' }}>"How Money Works" book</strong>, financial wellness tips, and occasional updates via email. I understand I can unsubscribe at any time.
                </span>
              </label>
            </div>
